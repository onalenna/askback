...
/**
 * Language the bot should reply in. Prefers a Whisper/hint code when it is
 * not a default-English guess that disagrees with the text.
 */
function resolveReplyLanguage(text, hinted) {
  ...
  if (fromHint && fromHint !== 'en') return fromHint;
  if (fromHint === 'en') return 'en';
  return '';
}

function userAskLanguage(text, { caption = '', hinted = '' } = {}) {
  const ask = String(caption || '')
    .replace(/@\d+/g, ' ')
  ...
  const first = String(text || '')
    ...
    .trim()
    .slice(0, 400);
  return resolveReplyLanguage(first || text, hinted);
}
