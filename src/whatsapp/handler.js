...
async function sendLinkText(sock, chatJid, links, { msg, mentions = [] } = {}) {
  if (!links?.length) return null;
  const { formatLinksMessage } = require('../ai/tts');
  const body = formatLinksMessage(links);
  const sent = await sock.sendMessage(chatJid, withMentions(body, mentions, sock, chatJid), {
    quoted: msg,
  });
  ...
  if (asVoice) {
    ...
    if (spoken) {
      ...
      if (sent) {
        ...
        if (links.length) await sendLinkText(sock, chatJid, links, { msg, mentions });
        return links.length ? 'voice+links' : 'voice';
      }
      console.warn('[whatsapp] voice reply failed — sending text');
    } else if (links.length) {
      await sendLinkText(sock, chatJid, links, { msg, mentions });
      return 'text';
    }
  }
